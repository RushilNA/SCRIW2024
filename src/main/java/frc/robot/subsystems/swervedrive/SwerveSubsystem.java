...
public class SwerveSubsystem extends SubsystemBase
{
   ...
   */
  public        double      maximumSpeed = Units.feetToMeters(25);
...
// Since AutoBuilder is configured, we can use it to build pathfinding commands
   ...
   */
   ...
   public Command driveCommand(DoubleSupplier translationX, DoubleSupplier translationY, DoubleSupplier angularRotationX) {
    return run(() -> {
        double xInput = Math.pow(translationX.getAsDouble(), 3) * swerveDrive.getMaximumVelocity();
        double yInput = Math.pow(translationY.getAsDouble(), 3) * swerveDrive.getMaximumVelocity();
        
        // Scale rotation more aggressively when moving to match the rotation-only speed
        double rotationInput = Math.pow(angularRotationX.getAsDouble(), 3) * swerveDrive.getMaximumAngularVelocity();
        
        // Increase rotation speed when the robot is translating
        if (Math.abs(xInput) > 0.1 || Math.abs(yInput) > 0.1) {
            rotationInput *= 1.5;  // Adjust the scaling factor (1.5) based on the effect
        }

        swerveDrive.drive(new Translation2d(xInput, yInput), rotationInput, true, false);
    });
}
  public Command driveCommand1(DoubleSupplier translationX, DoubleSupplier translationY, DoubleSupplier angularRotationX)
  {
    return run(() -> {
      // Make the robot move
